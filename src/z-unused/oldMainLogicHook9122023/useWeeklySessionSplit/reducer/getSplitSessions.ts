...
export const getSessionCounter = (
  ...
  sessions: SplitSessionsSessionsType
) => {
  switch (split) {
    case "BRO":
      return {
...
export const distributePPLULSplitAcrossWeek = <T>(
  week: TrainingDay[],
  sessions: {
    push: number;
    legs: number;
...
export const distributeULSplitAcrossWeek = (
  week: TrainingDay[],
  sessions: { upper: number; lower: number }
) => {
  let preferred: "upper" | "lower" =
    sessions.upper >= sessions.lower ? "upper" : "lower";
  let counter = { ...sessions };
