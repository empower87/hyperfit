import {
  ColumnDef,
  createColumnHelper,
  flexRender,
  getCoreRowModel,
  useReactTable,
} from "@tanstack/react-table";
import { useMemo } from "react";
import { BG_COLOR_M6, BG_COLOR_M7 } from "~/constants/themes";
import {
  ExerciseType,
  TrainingDayType,
} from "~/hooks/useTrainingProgram/reducer/trainingProgramReducer";
import { useTrainingProgramContext } from "~/hooks/useTrainingProgram/useTrainingProgram";

const fakeData: any = [];

const INITIAL_COLUMNS = [
  {
    header: "Day",
    accessor: "day",
  },
  {
    header: "Exercises",
    accessor: "exercises",
  },
];

type MicrocyclesType = {
  week: number;
  sets: number;
  reps: number;
  weight: number;
  rir: number;
};

type DataType = {
  split: string;
  exercise: {
    num: number;
    group: string;
    name: string;
    equipment: string;
    modality: string;
  };
  microcycles: MicrocyclesType[];
  deload: MicrocyclesType[];
};
const DATA: DataType = {
  split: "",
  exercise: {
    num: 0,
    group: "back",
    name: "pulldown",
    equipment: "machine",
    modality: "straight",
  },
  microcycles: [],
  deload: [],
};

const buildExerciseDataRow = (
  exercise: ExerciseType,
  index: number,
  split: string
) => {
  let data = { ...DATA };
  const details = exercise.mesocycle_progression.map((each, index) => {
    return {
      week: index,
      sets: each.sets,
      reps: each.reps,
      weight: each.weight,
      rir: each.rir,
    };
  });

  data = {
    ...data,
    split: split,
    exercise: {
      ...data.exercise,
      num: index,
      group: exercise.muscle,
      name: exercise.exercise,
      equipment: "dumbbell",
      modality: exercise.trainingModality,
    },
    microcycles: details,
    deload: [
      {
        ...details[details.length - 1],
        week: details[details.length - 1].week + 1,
      },
    ],
  };
  return data;
};

type SessionDataType = {
  split: string;
  exercises: DataType[];
};
const SESSION_DATA: SessionDataType = {
  split: "upper",
  exercises: [],
};

// export default function TrainingBlock() {
//   const { training_block, training_program_params } = useTrainingProgramContext();
//   const { mesocycles, microcycles } = training_program_params

//   return (
//     <div id="training_block" className={" flex flex-col"}>
//       {training_block.map((each, index) => {
//         return (
//           <Mesocycle
//             key={`${index}_${each[index]?.day}_mesocycles`}
//             split={each}
//             currentMesocycleIndex={index}
//           />
//         );
//       })}
//     </div>
//   );
// }

export default function TrainingBlock() {
  const { training_block, training_program_params } =
    useTrainingProgramContext();
  const { mesocycles, microcycles } = training_program_params;

  return (
    <div id="training_block" className={" flex flex-col"}>
      {/* {training_block.map((each, index) => {
        return (
          <Mesocycle
            key={`${index}_${each[index]?.day}_mesocycles`}
            split={each}
            currentMesocycleIndex={index}
          />
        );
      })} */}
      {training_block.map((each, index) => {
        return (
          <TrainingBlockTable
            mesocycle={each}
            index={index}
            microcycles={microcycles}
          />
        );
      })}
    </div>
  );
}
const columnHelper = createColumnHelper<DataType>();

const DELOAD = [
  columnHelper.group({
    header: `Deload`,
    columns: [
      columnHelper.accessor("microcycles.sets", {
        header: "sets",
        cell: (info) => info.getValue(),
      }),
      columnHelper.accessor("microcycles.reps", {
        header: "reps",
        cell: (info) => info.getValue(),
      }),
      columnHelper.accessor("microcycles.weight", {
        header: "weight",
        cell: (info) => info.getValue(),
      }),
      columnHelper.accessor("microcycles.rir", {
        header: "rir",
        cell: (info) => info.getValue(),
      }),
    ],
  }),
];

const defaultColumns = [
  columnHelper.group({
    header: "Exercise",
    columns: [
      columnHelper.accessor("exercise.num", {
        header: "#",
        cell: (info) => info.getValue(),
      }),
      columnHelper.accessor("exercise.group", {
        header: "Group",
        cell: (info) => info.getValue(),
      }),
      columnHelper.accessor("exercise.name", {
        header: "Name",
        cell: (info) => info.getValue(),
      }),
      columnHelper.accessor("exercise.equipment", {
        header: "Equipment",
        cell: (info) => info.getValue(),
      }),
      columnHelper.accessor("exercise.modality", {
        header: "Modality",
        cell: (info) => info.getValue(),
      }),
    ],
  }),
];

type TrainingBlockTableProps = {
  mesocycle: TrainingDayType[];
  index: number;
  microcycles: number;
};
function TrainingBlockTable({
  mesocycle,
  index,
  microcycles,
}: TrainingBlockTableProps) {
  const MICROCYCLE_COLUMNS = useMemo(
    () =>
      Array.from(Array(microcycles), (e, i) =>
        columnHelper.group({
          header: `Week ${i + 1}`,
          columns: [
            columnHelper.accessor((row) => row.microcycles[i].sets, {
              header: "sets",
              cell: (info) => info.getValue(),
            }),
            columnHelper.accessor((row) => row.microcycles[i].reps, {
              header: "reps",
              cell: (info) => info.getValue(),
            }),
            columnHelper.accessor((row) => row.microcycles[i].weight, {
              header: "weight",
              cell: (info) => info.getValue(),
            }),
            columnHelper.accessor((row) => row.microcycles[i].rir, {
              header: "rir",
              cell: (info) => info.getValue(),
            }),
          ],
        })
      ),
    [microcycles]
  );

  const data: DataType[] = useMemo(
    () =>
      mesocycle
        .map((each) => {
          const exercises = each.sessions
            .map((ea) => ea.exercises.flat())
            .flat();
          return exercises.map((e, i) =>
            buildExerciseDataRow(e, i + 1, each.sessions[0].split)
          );
        })
        .flat(),
    [mesocycle]
  );

  const columns: ColumnDef<DataType>[] = useMemo(
    () => [...defaultColumns, ...MICROCYCLE_COLUMNS, ...DELOAD],
    [defaultColumns, MICROCYCLE_COLUMNS, DELOAD]
  );

  const table = useReactTable({
    data,
    columns,
    getCoreRowModel: getCoreRowModel(),
  });

  return (
    <table className={`border-collapse`}>
      <thead>
        {table.getHeaderGroups().map((headerGroup, i) => (
          <tr key={`${headerGroup.id}_${index}_${i}`}>
            {headerGroup.headers.map((header, ii) => (
              <th
                key={`${header.id}_${index}_${ii}`}
                className={`text-xs text-white ${BG_COLOR_M7}`}
                colSpan={header.colSpan}
              >
                {header.isPlaceholder
                  ? null
                  : flexRender(
                      header.column.columnDef.header,
                      header.getContext()
                    )}
              </th>
            ))}
          </tr>
        ))}
      </thead>

      <tbody>
        {table.getRowModel().rows.map((row, i) => (
          <tr key={`${row.id}_${index}_${i}`} className={`${BG_COLOR_M6}`}>
            {row.getVisibleCells().map((cell, ii) => (
              <td
                key={`${cell.id}_${index}_${ii}`}
                className={`text-xxs text-white `}
              >
                {flexRender(cell.column.columnDef.cell, cell.getContext())}
              </td>
            ))}
          </tr>
        ))}
      </tbody>
    </table>
  );
}
